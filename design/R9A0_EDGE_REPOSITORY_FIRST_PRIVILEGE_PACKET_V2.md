# R9A0 Edge Repository-First Least-Privilege Packet V2

**Status:** `NON_EXECUTING_PRIVILEGE_PACKET_V2_READY_FOR_REREVIEW`  
...
**V1 review correction:** `MUNE-3313-001` — close effective function privileges against PostgreSQL `PUBLIC EXECUTE` and future default-privilege expansion.  
...
1. one monotonic Supabase migration defining or hardening `r9a0_edge_ingest`, closing effective `PUBLIC`/default function execution in `r9a0_api`, and applying the narrow explicit API grants;
2. one SQL privilege/negative-test suite proving the role cannot bypass the intended API surface now or after an unrelated future API function is added;
...
5. an explicit pre-apply inventory of every current `r9a0_api` function/procedure and every legitimate non-Edge execute principal whose access would be affected by removal of `PUBLIC EXECUTE`;
6. no Edge Function source, credential, deployment configuration, paid-service configuration, or hosted mutation in the same packet.
...
### 4.3 Function posture and effective ACL closure

PostgreSQL functions/procedures are executable by `PUBLIC` by default, and `r9a0_edge_ingest` is unavoidably a member of `PUBLIC`. A direct revoke from `r9a0_edge_ingest` therefore **cannot** override an effective grant inherited through `PUBLIC`. The V1 rule that revoked only Edge-specific functions was insufficient.

V2 binds the following closure strategy: **retain the existing `r9a0_api` schema, but eliminate `PUBLIC EXECUTE` as an ambient authority source for that schema and replace it with explicit named-principal grants.** A later implementation may not claim exact-function least privilege unless all of these conditions hold atomically in the reviewed migration:

1. inventory every current function and procedure in `r9a0_api`, including exact identity arguments/signatures, owner, security mode, current ACL, and effective execute principals;
2. identify every legitimate non-Edge consumer that currently depends on execution in `r9a0_api`; if that set cannot be enumerated exactly, hosted apply stops;
3. revoke `EXECUTE` from `PUBLIC` on every current function/procedure in `r9a0_api` before granting Edge access;
4. re-grant execution only to the explicit reviewed non-Edge principals that require each exact signature, preserving existing legitimate behavior without restoring `PUBLIC`;
5. grant `r9a0_edge_ingest` `EXECUTE` only on the exact Edge allowlist signatures below;
6. alter the **owning role's default privileges** for future functions/procedures in `r9a0_api` so new routines do not receive effective `PUBLIC EXECUTE`; this must cover the actual owner(s) that can create future API routines, not merely the migration runner;
7. prove that no alternate role membership, function ownership, schema privilege, provider default, or `PUBLIC` path restores execute rights to a non-allowlisted routine;
8. use fully qualified calls and a safe explicit `search_path`; attacker-controlled `public`, temporary, extension, or user schemas must not participate in routine resolution.
...
The exact symbol/signature must be bound to the product repository’s current database contract during implementation review. The implementation may not grant `EXECUTE ON ALL FUNCTIONS`, wildcard future-function rights, schema-wide default execute rights, or any `PUBLIC EXECUTE` fallback to `r9a0_edge_ingest`.

Illustrative posture, with exact owners/signatures resolved from the immutable implementation candidate:

```sql
-- Specification only. Exact owner and signatures are repository-bound.
REVOKE EXECUTE ON ALL FUNCTIONS IN SCHEMA r9a0_api FROM PUBLIC;

-- Apply for each role that owns or can create reviewed r9a0_api routines.
ALTER DEFAULT PRIVILEGES FOR ROLE r9a0_owner IN SCHEMA r9a0_api
  REVOKE EXECUTE ON FUNCTIONS FROM PUBLIC;

-- Re-grant only exact legitimate non-Edge principals/signatures after inventory.
-- GRANT EXECUTE ON FUNCTION r9a0_api.some_existing_fn(exact_types...) TO named_principal;

-- Edge receives only exact reviewed signatures.
-- GRANT EXECUTE ON FUNCTION r9a0_api.edge_ingest(exact_types...) TO r9a0_edge_ingest;
-- GRANT EXECUTE ON FUNCTION r9a0_api.edge_receipt(exact_types...) TO r9a0_edge_ingest;
```

`REVOKE EXECUTE ON ALL FUNCTIONS ... FROM PUBLIC` is a migration operation, not permission to break existing consumers. The migration is invalid unless the pre-apply inventory and explicit re-grants prove that every legitimate caller retains exactly the intended execution rights and no caller relies on ambient `PUBLIC EXECUTE`.
...
A future repository test suite is incomplete unless it proves the following using PostgreSQL **effective privilege** checks, not merely direct ACL inspection, as `r9a0_edge_ingest` and the named legitimate non-Edge principals.
...
- execute on any `r9a0_api` function/procedure outside the exact allowlist when privileges are computed effectively through direct grants, `PUBLIC`, ownership, and role membership;
- any residual `PUBLIC EXECUTE` on a current `r9a0_api` routine after the migration;
- wildcard/function-default expansion after an unrelated API function is added by each reviewed owning role;
- execute on a newly created unrelated `r9a0_api` function before any explicit grant is issued;
- loss of execution for any legitimate named non-Edge principal whose exact pre-migration access was intentionally preserved by the reviewed re-grant matrix;
...
- every current `r9a0_api` routine has `PUBLIC EXECUTE = false` after the migration unless an independently reviewed exception explicitly proves it cannot widen Edge authority; the default position is no exceptions;
- owner-scoped default privileges are read back and prove that a newly created unrelated API routine is not executable by `PUBLIC` or `r9a0_edge_ingest` before an explicit grant;
- the explicit non-Edge re-grant matrix preserves exactly the reviewed legitimate existing callers and no ambient `PUBLIC` fallback;
- `r9a0_edge_ingest` has zero direct relation/sequence rights and only the exact function/procedure allowlist through effective privilege evaluation;
...
- complete `r9a0_api` routine inventory digest, including exact signatures, owners, security modes, and effective execute principals before/after;
- explicit legitimate non-Edge re-grant matrix digest;
- owner-scoped default-privilege readback proving future routines do not regain `PUBLIC EXECUTE`;
...
`REPOSITORY_FIRST_PRIVILEGE_PACKET_V2_READY_FOR_REREVIEW`

This V2 corrects `MUNE-3313-001` and remains the assignment 3313 design-contract candidate pending independent rereview. It authorizes **no** migration apply, role/credential creation, hosted mutation, Edge deployment, paid-service action, production mutation, or `service_role` use.
